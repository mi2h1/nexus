...
#### 2026-03-17 (v0.2.29: 画面共有 適応型品質・凍り対策)
- **`adaptiveStream: true`** を LiveKit Room に追加 — 帯域不足時に自動で低品質層へ切り替え（凍り防止）
- **サイマルキャスト層を多段化**: 選択プリセットに応じた段階的フォールバック層を定義
  - 低画質(720p/15): [480p/15, 480p/30]
  - 標準(720p/30): [480p/15, 480p/30, 720p/15]
  - 高画質(1080p/30): [480p/15, 480p/30, 720p/15, 720p/30]
  - 配信向け(1080p/60): [480p/15, 480p/30, 720p/15, 720p/30, 1080p/30]
- ユーザー向けの品質プリセット選択UI は変更なし
...
#### 2026-03-17 (v0.2.25: ボイスEQ・マイクテストVUメーター・ゲートポップ修正)

- **ボイス EQ 機能追加**:
  - オート（固定プリセット: 350Hz -3dB / 3kHz +2.5dB）/ シンプル（4バンド縦スライダー）/ カスタム（8バンド縦スライダー）の3モード
  - ピル型セグメントコントロールでモード切替（切替時にゲート中断なし、`applyEqBands()` でリアルタイム反映）
  - シンプル/カスタム初期値はオートと同等の音になるよう設定済み
  - モード・各バンドゲインは `nexus_eq_mode` / `nexus_eq_simple_gains` / `nexus_eq_custom_gains` に永続化
  - ヘルプボタン（`HelpIcon`）でモーダル説明を表示
  - グローバル `.mx_Dialog button:not(...)` スタイルの除外リストに `nx_VoiceSettings_eqHelpBtn` を追加
- **マイクテスト VU メーター刷新**:
  - Discord スタイルの横並び縦棒メーター（64本、等幅・等高）
  - 左→右の点灯で音量レベルを表現、点灯色は黄（左）→緑（右）グラデーション
  - モニターOFF / 非VC時は消灯（`monitorEnabled` ガード追加）
- **ボイスゲート ポップ音修正**:
  - ゲートオープン時を即座(`setValueAtTime`)から 10ms フェードイン(`linearRamp`)に変更
  - ルックアヘッド 15ms 以内に収まるため音頭欠けなし
